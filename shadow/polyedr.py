...
class Polyedr:
    ...
    def __init__(self, file):
        ...
        with open(file) as f:
            for i, line in enumerate(f):
                if i == 0:
                    buf = line.split()
                    self.c = float(buf.pop(0))          # коэффициент гомотетии только для рисования
                    alpha, beta, gamma = (float(x) * pi / 180.0 for x in buf)
                elif i == 1:
                    nv, nf, ne = (int(x) for x in line.split())
                elif i < nv + 2:
                    x, y, z = (float(x) for x in line.split())
                    # Вершины сохраняются БЕЗ умножения на c
                    self.vertexes.append(R3(x, y, z).rz(alpha).ry(beta).rz(gamma))
                else:
                    buf = line.split()
                    size = int(buf.pop(0))
    ...
    def draw(self, tk): # pragma: no cover
        ...
        for e in self.edges:
            for s in e.gaps:
                # гомотетия применяется только здесь
                beg = e.r3(s.beg) * self.c
                fin = e.r3(s.fin) * self.c
                tk.draw_line(beg, fin)